...
Using_SVAR = False
PreFix = 'PCMCI_usnig_tigramite_VAR_'
...
def dict_to_list(root,dict,W):
    lis = []
    for node in dict.keys():
        lis.append(((node-1,-1),round(W[root-1,node-1 ],1)))
    return lis

parents_neighbors_coeffs = {}
for node in  GT.keys():
    parents_neighbors_coeffs[node-1] = dict_to_list(node,GT[node],W)



'''VAR'''
data, _ = toys.var_process(parents_neighbors_coeffs, T=8000)
