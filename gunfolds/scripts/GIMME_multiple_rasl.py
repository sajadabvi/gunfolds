...
PreFix = 'GIMME_hardcode_selfloop_add_bidir_rasl'
concat = True
...
def read_csv_files_for_rasl(path,size):
    files = sorted(os.listdir(path))  # Sort the files
    for filename in files:
        if filename.endswith('.csv'):
            file_path = os.path.join(path, filename)
            with open(file_path, 'r') as file:
                ...
                matrix1 = np.array(mat[:, 0:size])
                matrix2 = np.array(mat[:, size:2*size])
                A = (matrix1 != 0).astype(int)
                B = (matrix2 != 0).astype(int)
    return A.T, B.T
...
for nn in [1,2,3,4,5,6]:
    ...
    for fl in range(1, 2):
        path = "/Users/sajad/GSU Dropbox Dropbox/Mohammadsajad Abavisani/Mac/" \
               "Documents/PhD/Research/code/GIMME/gimme-master/gimme-master/" \
               +str(nn)+"_05VARfalse/sum"
        csv_data = read_csv_files(path,len(network_GT))
        ...
        MVGC = cv.adjs2graph(csv_data.T, B)
        ...
        # ###trying sRASL
        A,B = read_csv_files_for_rasl(path,len(network_GT))
        GIMME_rasl = cv.adjs2graph(A,B)
        nx_MVGC = gk.graph2nx(GIMME_rasl)
        two_cycle = mf.find_two_cycles(nx_MVGC)
        DD = np.ones((len(network_GT), len(network_GT))) * 10000
        ...
        edge_weights = [1, 3, 1, 3, 2]
        r_estimated = drasl([GIMME_rasl], weighted=True, capsize=0,
                            urate=min(5, (3 * len(GIMME_rasl) + 1)),
                            scc=False,
                            dm=[DD],
                            bdm=[BD],
...
###saving files
filename = PreFix + '_lag_only_cutoff05_prior_'+''.join(map(str, edge_weights))+'_with_selfloop_net_' + str('all') + '_amp_' + now + '_' + (
