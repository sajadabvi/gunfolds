...
for nn in [4]:
    ...
    for fl in range(1, 61):
        ...
        MVAR = cv.adjs2graph(mat, np.zeros((len(network_GT), len(network_GT))))
        normal_GT = mf.precision_recall(MVAR, network_GT, include_selfloop = include_selfloop)
        ...
        undersampled_GT = mf.precision_recall(MVAR, new_GT,include_selfloop = include_selfloop)
        ...
        nx_MVAR = gk.graph2nx(MVAR)
        two_cycle = mf.find_two_cycles(nx_MVAR)
        ...
        for i in range(len(network_GT)):
            DD[i][i] = 10000
        MVAR_bi = cv.adjs2graph(mat, B)
        individuals.append(MVAR_bi)
        # gt.plotg(MVAR_bi, output='./figs/cycle_removed/Gopt_GC_' + str(nn*1000+fl) + '.pdf')
        edge_weights = [1, 3, 1, 3, 2]
        r_estimated = drasl([MVAR_bi], weighted=True, capsize=0,
                            urate=min(5, (3 * len(MVAR_bi) + 1)),
                            scc=False,
                            dm=[DD],
                            bdm=[BD],
...
for i, (
    ...
    # ax3.set_ylim(0, 1)
# Add super title
plt.suptitle('MVAR, TR= ' + TR + ' ' + ('concat' if concat else 'individual') + ' data')
# Legend
...
red_patch = mpatches.Patch(color='red', label='MVAR+bi+sRASL')
