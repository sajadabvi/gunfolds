...
parser.add_argument("-m", "--MODEDOUBLE",default="f", help="whether or not use two input for d_rasl. Use y for true and n for false.", type=str)
...
def clingo_caller(g):
    ...
    c = drasl(g, capsize=args.CAPSIZE, urate=min(30,(3*len(g[0])+1)), timeout=TIMEOUT, scc=SCCMODE, pnum=args.PNUM, weighted=True)
