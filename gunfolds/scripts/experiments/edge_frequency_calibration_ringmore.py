"""
Edge-Level Frequency vs. Correctness Calibration — Ring+Random Graphs

Same calibration analysis as edge_frequency_calibration.py but using randomly
generated ring graphs (via ringmore) instead of the fixed Sanchez-Romero
networks.  Graph sizes and target directed-edge densities are swept as
experiment parameters.

For each (n_nodes, density, batch) combination this script:
1. Generates a ring graph with additional random directed edges to reach
   the target density (using graphkit.ringmore)
2. Creates a stable weighted transition matrix and generates synthetic data
3. Runs PCMCI to get an initial graph estimate
4. Runs DRASL (with self-loops enabled) to get the solution set
5. Computes per-edge frequency across solutions
6. Bins edges by frequency and computes precision per bin
7. Produces a calibration curve: edge agreement frequency vs. correctness
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Edge frequency calibration with ring+random graphs '
                    '(SLURM array compatible).')

    sub = parser.add_subparsers(dest='mode', help='Execution mode')

    shared = argparse.ArgumentParser(add_help=False)
    shared.add_argument("-n", "--NODE_SIZES", nargs='+',
                        default=[5, 6, 7, 8], type=int,
                        help="graph sizes (number of nodes)")
    shared.add_argument("-d", "--DENSITIES", nargs='+',
                        default=[0.2, 0.25, 0.3], type=float,
                        help="target directed-edge densities")
    shared.add_argument("-u", "--UNDERSAMPLING", nargs='+',
                        default=[2, 3], type=int,
                        help="undersampling rates")
    shared.add_argument("-b", "--BATCHES", default=10, type=int,
                        help="number of random-graph batches per configuration")
    ...
    shared.add_argument("--output_dir",
                        default="results_edge_calibration_ringmore",
...
def run_single_experiment(n_nodes, target_density, u_rate, batch_idx,
                          ssize, noise, timeout_hours, pnum,
    ...
    try:
        ...
        print(f"  Skipping n={n_nodes}, d={target_density}, u={u_rate}, "
              f"batch={batch_idx}: could not create stable matrix")
        return None
    ...
    if len(r) == 0:
        print(f"  No solutions for n={n_nodes}, d={target_density}, "
              f"u={u_rate}, batch={batch_idx}")
        return None
...
def print_summary(directed_cal, bidirected_cal):
    ...
    print("\n" + "=" * 60)
    print("CALIBRATION SUMMARY (All Edges) — Ring+Random Graphs")
    print("=" * 60)
...
def mode_run(args):
    ...
    print(f"[Task {args.task_id}/{total_tasks - 1}] "
          f"n={n_nodes}, density={density}, u={u_rate}, batch={batch}")

    ignore_sl = not args.include_selfloops
    result = run_single_experiment(
        n_nodes, density, u_rate, batch,
        args.ssize, args.noise, args.TIMEOUT, args.PNUM,
    ...
    if result is not None:
        print(f"  Solutions: {result['n_solutions']}, "
              f"dir_edges: {len(result['directed_calibration'])}, "
              f"bidir_edges: {len(result['bidirected_calibration'])}")
    else:
...
def mode_aggregate(args):
    ...
    save_path = os.path.join(agg_dir,
                             'edge_calibration_results_ringmore.zkl')
    zkl.save(save_data, save_path)
    print(f"Aggregated results saved to {save_path}")

    plot_path = os.path.join(agg_dir,
                             'edge_frequency_calibration_ringmore.pdf')
...
def mode_local(args):
    ...
    for idx, (n_nodes, density, u_rate, batch) in enumerate(tasks):
        print(f"[{idx + 1}/{total}] n={n_nodes}, d={density}, "
